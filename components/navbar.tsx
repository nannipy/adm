...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <nav className="hidden md:flex bg-white  rounded-full m-4 mx-40">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white p-4 rounded-3xl m-4 shadow-md z-50">
        <div className="flex justify-around items-center">
          ...
          <SignedIn>
            ...
            <Link href={`/workout/${user?.id}`} className={`text-black p-5 rounded-2xl hover:bg-gray-100 ${isActive(`/workout/${user?.id}`)}`}>
              <Dumbbell size={24} />
            </Link>
            <Link href={`/scheda/${user?.id}`} className={`text-black p-5 rounded-2xl hover:bg-gray-100 ${isActive(`/scheda/${user?.id}`)}`}>
              <FileText size={24} />
            </Link>
            <Link href={`/manual/${user?.id}`} className={`text-black p-5 rounded-2xl hover:bg-gray-100 ${isActive(`/manual/${user?.id}`)}`}>
              <BookMarked size={24} />
              </Link>
              <Link href={`/messages/${user?.id}`} className={`text-black p-5 rounded-2xl hover:bg-gray-100 ${isActive(`/messages/${user?.id}`)}`}>
                <MessageSquare size={24}/>
              </Link>
            {isAdmin && (
              <Link href="/admin/create-workout" className={`text-black p-5 rounded-2xl hover:bg-gray-100 ${isActive('/admin/create-workout')}`}>
                <Settings size={24} />
              </Link>
            )}
            <Link href={`/user/${user?.id}`} className={`text-black p-5 rounded-2xl hover:bg-gray-100 ${isActive(`/user/${user?.id}`)}`}>
              <User size={24} />
            </Link>
          </SignedIn>
          <SignedOut>
